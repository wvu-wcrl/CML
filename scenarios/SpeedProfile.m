...
sim_param(record).timing_sample_rate = 5;
...
sim_param(record).save_rate = 100;
sim_param(record).MaxRunTime = 3*60; 
sim_param(record).timing_sample_rate = 20; 
sim_param(record).ProfileSpeed = 1;
...
sim_param(record).MaxRunTime = 10*60; 
...
% convolutional code
record = 4;
sim_param(record).comment = 'Rate 1/2 K=7 NSC convolutional code w/ BPSK in Rayleigh Fading';
sim_param(record).sim_type = 'coded';
