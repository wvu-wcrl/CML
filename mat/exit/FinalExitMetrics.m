...
switch sim_param.exit_param.exit_type,
    case 'ldpc',
        ...
        for k = 1:length( sim_param.exit_param.requested_IA )
            ...
            sim_state.exit_state.I_A_det(k,snrpoint) =J2( sqrt(sim_param.exit_param.dv) * sigma_IA); % equation 20 in Brink paper, 2004
            ...
            sigma = (sim_param.exit_param.dv-1)* sigma_IA^2 + (invJ1(sim_state.exit_state.I_E_det(k,snrpoint)))^2;
            sim_state.exit_state.IE_vnd(k,snrpoint) = J2(sqrt(sigma));  % equation 22 in Brink paper, 2004
        end
